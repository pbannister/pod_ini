...
struct base_hash::value_o {
    node_o* p_node = 0;
    value_o* p_value_next = 0;
};

struct base_hash::section_o {
    string_o key;
...
base_hash::section_o* base_hash::tree_root_o::section_find(const char* name) {
    for (auto p_section = p_section_head; p_section; p_section = p_section->p_section_next) {
        if (0 == ::strcmp(p_section->key, name)) {
            return p_section;
        }
    }
    return 0;
...
const char* base_hash::value_find(section_o* p_section, const char* name, const char* _value) {
    for (auto p_value = p_section->p_value_head; p_value; p_value = p_value->p_value_next) {
        auto p_node = p_value->p_node;
        if (0 == ::strcmp(p_node->key, name)) {
            return p_node->value;
        }
    }
    return _value;
...
void base_hash::tree_root_o::tree_print_pod_struct() {
    ...
    for (auto p_section = p_section_head; p_section; p_section = p_section->p_section_next) {
        ...
        ::printf("\n    struct configuration_pod_%s_t {", section_name.buffer_get());
        for (auto p_value = p_section->p_value_head; p_value; p_value = p_value->p_value_next) {
            auto key1 = p_value->p_node->key.buffer_get();
            auto key2 = (::strchr(key1, '!') + 1);
            ::printf("\n        const char* v_%s;", key2);
        }
        ::printf("\n    } s_%s;", section_name.buffer_get());
    }
    ::printf("\n} configuration_pod;\n");
}
...
void base_hash::tree_root_o::tree_print_pod_initialize() {
    ...
    for (auto p_section = p_section_head; p_section; p_section = p_section->p_section_next) {
        string_o section_name(p_section->key);
        string_filter(section_name.buffer_get(), '.', '_');
        ::printf("\n    { // %s", section_name.buffer_get());
        for (auto p_value = p_section->p_value_head; p_value; p_value = p_value->p_value_next) {
            auto key1 = p_value->p_node->key.buffer_get();
            auto key2 = (::strchr(key1, '!') + 1);
            string_o quoted;
            string_requote(p_value->p_node->value, quoted);
            ::printf("\n        \"%s\", // %s", quoted.buffer_get(), key2);
        }
        ::printf("\n    },");
    }
    ::printf("\n};\n");
...
void base_hash::tree_root_o::tree_print_pod_reader() {
    ...
    for (auto p_section = p_section_head; p_section; p_section = p_section->p_section_next) {
        string_o section_name(p_section->key);
        string_filter(section_name.buffer_get(), '.', '_');
        const char* s_section = section_name;
        ::printf(
            "\n    {"
            "\n        auto p_section = p_context->tree.section_find(\"%s\");"
            "\n        if (p_section) {",
            s_section);
        for (auto p_value = p_section->p_value_head; p_value; p_value = p_value->p_value_next) {
            auto key1 = p_value->p_node->key.buffer_get();
            auto key2 = (::strchr(key1, '!') + 1);
            const char* v_name = key2;
            ::printf(
                "\n            configuration_pod.s_%s.v_%s = base_hash::value_find(p_section, \"%s\", configuration_pod.s_%s.v_%s);",
                s_section, v_name,
                v_name,
                s_section, v_name);
        }
